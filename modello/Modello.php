...
//----------------- funzioni relative alle PRENOTAZIONI  -------------------------------
	...
	{
		include 'controllo/connect_db.php';
        $query_num_biglietti="SELECT SUM(num_biglietti) AS tot_biglietti FROM prenota WHERE utente = $id_utente";
        $num_prenotazioni=mysqli_query($connessione, $query_num_biglietti);
        $n_prenotazioni = mysqli_fetch_assoc($num_prenotazioni);
        return $n_prenotazioni['tot_biglietti'];
	}

	public function recupera_num_prenotazioni_per_idArticolo($id_articolo)
	{
		include 'controllo/connect_db.php';
        $query_num_biglietti="SELECT utente FROM prenota WHERE articolo = $id_articolo";
        $num_prenotazioni=mysqli_query($connessione, $query_num_biglietti);
